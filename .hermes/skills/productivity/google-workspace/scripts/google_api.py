...
def tasks_list(args):
    service = build_service("tasks", "v1")
    params = {"tasklist": args.tasklist, "maxResults": args.max}
...
def tasks_create(args):
    ...
    if args.due:
        # Tasks API requires RFC 3339 with Z; date-only allowed via T00:00:00.000Z
        body["due"] = args.due
    created = service.tasks().insert(tasklist=args.tasklist, body=body).execute()
    print(json.dumps({"status": "created", "id": created["id"], "title": created.get("title", "")}, indent=2))
...
def tasks_complete(args):
    ...
    updated = service.tasks().update(tasklist=args.tasklist, task=args.task_id, body=task).execute()
    print(json.dumps({"status": "completed", "id": updated["id"]}, indent=2))
...
def main():
    ...
    p = tsk_sub.add_parser("lists", help="List task lists")
    p.add_argument("--max", type=int, default=20)
    p.set_defaults(func=tasks_lists)

    p = tsk_sub.add_parser("list", help="List tasks in a task list")
    p.add_argument("--tasklist", default="@default", help="Task list ID (default: @default)")
    p.add_argument("--max", type=int, default=100)
    p.add_argument("--show-completed", action="store_true")
    p.set_defaults(func=tasks_list)

    p = tsk_sub.add_parser("create", help="Create a task")
    p.add_argument("--title", required=True)
    p.add_argument("--notes", default="")
    p.add_argument("--due", default="", help="RFC 3339 datetime, e.g. 2026-04-25T00:00:00.000Z")
    p.add_argument("--tasklist", default="@default")
    p.set_defaults(func=tasks_create)

    p = tsk_sub.add_parser("complete", help="Mark a task complete")
    p.add_argument("task_id")
    p.add_argument("--tasklist", default="@default")
    p.set_defaults(func=tasks_complete)

    p = tsk_sub.add_parser("delete", help="Delete a task")
    p.add_argument("task_id")
    p.add_argument("--tasklist", default="@default")
